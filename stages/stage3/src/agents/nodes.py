...
def reservation_node(state: dict) -> dict:
    ...
    if step == "confirm" and text.lower() in ("confirm", "yes", "ok", "y"):
        reservation_id = run_admin_agent(state.get("thread_id", str(uuid.uuid4())), data)
        return {
            "collection_step": "pending_approval",
            "reservation_data": data,
            "reservation_id": reservation_id,
            "approval_status": "pending",
            "response": f"Your reservation request was sent to the administrator. Reservation ID: {reservation_id[:8].upper()}",
        }
    return {"collection_step": "confirm", "reservation_data": data, "response": _build_summary(data)}
