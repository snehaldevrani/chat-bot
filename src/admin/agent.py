...
@tool
def save_reservation_tool(thread_id: str, name: str, surname: str, car_number: str,
                           start_datetime: str, end_datetime: str, space_type: str) -> str:
    """Save a confirmed reservation to the database. Returns the reservation_id."""
    rid = str(uuid.uuid4())
    data = {
        "name": name, "surname": surname, "car_number": car_number,
        "start_datetime": start_datetime, "end_datetime": end_datetime,
        "space_type": space_type,
    }
    create_reservation(rid, thread_id, data)
    return rid


@tool
def notify_admin_tool(reservation_id: str, name: str, surname: str, car_number: str,
                       start_datetime: str, end_datetime: str, space_type: str) -> str:
    ...
    sent = send_approval_email(reservation_id, data)
...
def check_status_tool(reservation_id: str) -> str:
    """Check the current approval status of a reservation."""
...
def run_admin_agent(thread_id: str, reservation_data: dict) -> str:
    ...
    # Use json.dumps to safely serialize user-supplied data — prevents prompt injection
    # via crafted names, car numbers, or datetimes containing quote/newline characters.
    ...
    # Extract UUID from agent response
